...
- **Draft FC105 (Glue Scale Logic)** added: `plc-drafts/FC105.awl` +
  `FC105_design.md`. Called from OB1 (it makes the actual QC decision, so
  ...
  Flags an open naming question: FC105 is Siemens' standard-library SCALE
  block number, so this needs confirming as custom vs. the standard block
  before trusting it.
