...
int Adafruit_MLX90640::MLX90640_I2CRead(uint8_t slaveAddr,
                                        ...
                                        uint16_t *data) {
  // REVERTED (see project notes): an earlier version of this function
  // wrote the register pointer once and relied on
  // Adafruit_I2CDevice::read()'s internal chunking (bare requestFrom()
  // calls chained with stop=false between them, no write in between)
  // to cover the full length in one call. That produced deterministic
  // garbage frame data on this ESP32-S3 hardware -- write-then-read
  // repeated-start (endTransmission(false) -> requestFrom()) is the
  // well-supported pattern; chaining read-after-read with stop=false
  // and no intervening write apparently is not. Confirmed by swapping
  // back to the stock library, which produced correct, physically
  // sane temperatures (21-26 C) on the exact same hardware where the
  // modified version produced fixed, wrong values (75/973 C) on every
  // run. Back to the original, proven-correct chunked implementation.
  uint8_t cmd[2];
...
/*!
 *    @brief  QC-510 ADDITION: read one full frame (both subpages) of
 *            RAW ADC pixel counts, skipping MLX90640_GetTa() and
 *            MLX90640_CalculateTo() entirely. See the header comment on
 *            this method for the fixed-pattern-noise caveat before
 *            using this data for threshold detection.
 *    @param  frameData0 uint16_t[834] buffer for subpage 0's raw data
 *    @param  frameData1 uint16_t[834] buffer for subpage 1's raw data
 *    @return 0 on success, matching getFrame()'s convention
 */
int Adafruit_MLX90640::getRawFrame(uint16_t *frameData0,
                                   uint16_t *frameData1) {
  int status;

  status = MLX90640_GetFrameData(0, frameData0);
  if (status < 0) {
    return status;
  }

  status = MLX90640_GetFrameData(0, frameData1);
  if (status < 0) {
    return status;
  }
