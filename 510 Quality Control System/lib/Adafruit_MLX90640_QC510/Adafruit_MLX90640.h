...
public:
  ...
  // QC-510 ADDITION: read one full frame (both subpages) of RAW,
  // uncalibrated ADC pixel counts -- skips MLX90640_GetTa() and
  // MLX90640_CalculateTo() entirely. frameData0/frameData1 must each
  // point to a uint16_t[834] buffer (same 834-word layout the stock
  // library uses internally: [0..767] pixel counts, [768..833] header/
  // aux words including gain at [778] and subpage index at [833]).
  // Returns 0 on success, matching getFrame()'s convention.
  //
  // CAVEAT (bench-validate before trusting thresholds on this data):
  // this is the sensor's raw ADC output with NO per-pixel offset/gain
  // correction applied. Each pixel has its own EEPROM-stored offset and
  // gain trim -- that's real fixed-pattern noise across the 32x24 array,
  // not just sensor noise. Thresholding raw counts directly may need a
  // per-pixel baseline capture (e.g. average N frames at idle) subtracted
  // out before max-hold, the same way CAPTURE_TRIGGER_TEMP_C needed
  // validation against real glue thermal signatures. Existing
  // MATRIX_TEMP_MAX_C / CAPTURE_TRIGGER_TEMP_C constants (in degrees C)
  // do NOT carry over to this domain -- new raw-count thresholds need to
  // be derived empirically.
  int getRawFrame(uint16_t *frameData0, uint16_t *frameData1);
